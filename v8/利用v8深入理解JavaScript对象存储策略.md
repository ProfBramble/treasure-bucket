...
> 本文是系列的第二篇，包含部分计算机系统知识，且所有内容从 [利用 v8 深入理解 JavaScript 设计](https://zhuanlan.zhihu.com/p/361787647) 一文继续延伸，如读者发现文中错误可以在评论区指出或联系笔者
...
接下来我们观察下列输出的对象信息，可以发现无论是什么形式的对象，在 V8 中存储格式都是一样的，其中
...
之后我们没有按照创建隐藏类的顺序去删除属性，直接删除 name 属性后，隐藏类被重新创建。这里有个非常重要的细节，那就是隐藏类的 `back pointer` 会和我们一开始声明 `const foo = {}` 时一致，可以理解我们重新创建了一个隐藏类根节点。这样一个不经意的动作会带来远比之前顺序操作昂贵得多的代价。
...
在系列第一篇文章中，笔者说对象内属性在对象创建的时候默认有 10 个，当时只是按照用例进行描述的，真正的对象内属性数量在对象定义的时候就已经决定了，而且一旦声明完毕后就无法改变。本期介绍隐藏类后我们可以知道，对象内属性的具体数量在它存在之时已经被记录在内部的描述信息中。因此只要我们定义的初始对象属性数量够多，那么对应的对象内属性的个数也会被分配的更多。
...
一般情况下对象的属性都默认是快属性，但是如果频繁修改对象（包括添加与删除属性），那么就会伴随大量时间和内存开销维护隐藏类，这让隐藏类带来的性能加速显得得不偿失，因此 V8 才会引入慢属性。慢属性直接存储在单独的属性字典结构中，并且与隐藏类断开联系，在慢属性中的对象修改行为都不会更新隐藏类，因此内联缓存（inline-cache）策略也不适用于慢属性，缺点显而易见，就是读取速度很慢。
